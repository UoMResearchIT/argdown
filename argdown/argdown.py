...
argparser.add_argument('--header', type=str, default='Arguments and Usage',
...
argparser.add_argument('-d', '--hide-default', dest='show_default', action='store_false',
...
argparser.add_argument('--header-depth', dest='depth', type=int, default=1,
...
argparser.add_argument('-v', '--version', action='version',
    version=f'%(prog)s {importlib.metadata.version(argparser.prog)}')
...
def gen_help(lines, function = None, **kwargs):
    indent = 0
    ...
    args = ', '.join(args)
    lines.append(' ' * indent + f'print(md_help({args}))')        
...
def console(args=None):
    ...
    md_help_args = [
        p.name for p in inspect.signature(md_help).parameters.values()
        if p.kind == p.KEYWORD_ONLY
    ]
    bad_arguments = [k for k in md_help_args if k not in vars(args)]
    if bad_arguments:
        raise ValueError(f'Invalid arguments passed to md_help: {bad_arguments}')

    cfg = {k: v for k, v in vars(args).items() if v is not None and k in md_help_args}
    
    if args.use_stdin:
        # catenate stdinput, parse / render
        gen_help(sys.stdin.readlines(), **cfg)
        return

    # process each file, respecting encoding
    for fname in args.src_file:
        with open(fname, 'r', encoding=args.encoding) as f:
            gen_help(f.readlines(), **cfg)